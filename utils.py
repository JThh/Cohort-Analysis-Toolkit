...
class CohortAnalyzer():
    ...
    def integrate_module_information(self):
        ...
        mod_info_agg = mod_info.groupby(['mod_code']).agg({'grading_basis':' '.join,'mod_faculty':max,'mod_activity_type':','.join,'mod_level':max})
    ...
    def plot_most_different_modules(self, n=10):
        '''
        Plot the module enrolment sorted by normalized enrolment difference between the two academic years.
    ...
    def PCAnalysis(self, n_components=5, n_iters=7, random_state=47, topkmods=10):
        ...
        module_set = list(set(stu_mod_1.mod_code).union(set(stu_mod_1.mod_code)))
        ...
        pcs_1 = svd_1.components_
        pcs_2 = svd_2.components_
        ...
        fig = px.bar(mod_diff_svd_info.iloc[:topkmods,:], x='index', y='pc_diff',hover_data=['mod_code','grading_basis','mod_faculty'])
        fig.show()     
        print("It is advised to clear the cache each time after running a graphing function!")
    ...
    def attr_perc_change(self, attr='mod_faculty'):
        ...
        mod_focus = self.mod_code_sorted_by_diff.merge(self.mod_info,on=attr).astype({attr: 'category'})
        mod_focus_grouped_1 = mod_faculty[[self.coht1,attr]].groupby([attr]).sum()
        mod_focus_grouped_2 = mod_faculty[[self.coht2,attr]].groupby([attr]).sum()
        ...
        fig = px.bar(mod_faculty_combined,x=attr, y='percentage_change',hover_data={self.coht1:True,self.coht2:True,'percentage_change':':.2f'})
        fig.show()   
